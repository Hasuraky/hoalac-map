...
export default function LeadPage() {
  ...
  const [sales, setSales] = useState([]); // danh sách sale để phân (admin/owner)
  ...
  useEffect(() => {
    fetchRole().then((r) => {
      setRole(r);
      if (['sale', 'admin', 'owner'].includes(r)) {
        load();
        if (r === 'admin' || r === 'owner') loadSales();
  ...
  if (!['sale', 'admin', 'owner'].includes(role)) {
    return (
      <div className="detail-page">
        ...
        <p className="form-notice">Trang này dành cho nhân viên tư vấn.</p>
      </div>
    );
  }
  ...
  return (
    <div className="detail-shell">
      <header className="detail-nav">
        ...
        <span className="code">Yêu cầu tư vấn</span>
      </header>
            ...
            {leads.map((l) => (
              <div className="detail-card lead-item" key={l.id}>
                <div className="lead-head">
                  ...
                  <span className={`role-badge lead-status-${l.status}`}>
                ...
                {/* Admin/Owner: phân sale khi lead ở kho */}
                ...
                {/* Sale (lead của mình) hoặc owner: đổi trạng thái */}
                {(role === 'owner' || (role === 'sale' && l.status === 'assigned')) && (
                  <div className="lead-actions">
                    <button className="btn-mini" disabled={busy} onClick={() => patch(l.id, { status: 'converted' })}>
                      ✓ Đã thành khách
                    </button>
                    <button className="btn-mini" disabled={busy} onClick={() => patch(l.id, { status: 'returned' })}>
                      ↩ Trả lại kho
                    </button>
                    <button className="btn-mini danger" disabled={busy} onClick={() => patch(l.id, { status: 'spam' })}>
                      🗑 Data rác
                    </button>
                  </div>
                )}

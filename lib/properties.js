...
// Chạy promise với giới hạn thời gian -> không để một lời gọi treo làm kẹt cả trang
...
// Dùng getSession() (đọc từ bộ nhớ, không gọi mạng) thay cho getUser() để tránh treo khi tải trang.
// An toàn: quyền thật vẫn do RLS phía máy chủ quyết định — đây chỉ dùng để chọn view hiển thị.
export async function fetchRole() {
  if (!supabase) return 'admin'; // demo: xem đầy đủ
  const {
    data: { session },
  } = await withTimeout(supabase.auth.getSession(), 3000, { data: { session: null } });
